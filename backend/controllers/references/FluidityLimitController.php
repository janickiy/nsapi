...
use backend\services\references\FluidityLimitService;
...
class FluidityLimitController extends Controller
{
    use BaseExceptionProcessingErrorsTrait;

    const GRID_NAME = 'grid_fluidity_limit';
    ...
    {
        $searchModel = new FluidityLimitSearch();
    ...
    {
        $model = new FluidityLimit();
        $modelForm = new FluidityLimitForm();
        if ($modelForm->load(Yii::$app->request->post()) && $modelForm->validate()) {
            try {
                $service = new FluidityLimitService($model);
     ...
     * @throws NotFoundHttpException
     */
    private function findModel($id): FluidityLimit
    {
        $model = FluidityLimit::findOne(intval($id));
        if (!$model) {
