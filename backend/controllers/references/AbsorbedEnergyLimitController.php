...
use backend\services\references\AbsorbedEnergyLimitService;
...
class AbsorbedEnergyLimitController extends Controller
{
    use BaseExceptionProcessingErrorsTrait;

    const GRID_NAME = 'grid_absorbed_energy_limit';
    ...
    {
        $searchModel = new AbsorbedEnergyLimitSearch();
    ...
    {
        $model = new AbsorbedEnergyLimit();
        $modelForm = new AbsorbedEnergyLimitForm();
        if ($modelForm->load(Yii::$app->request->post()) && $modelForm->validate()) {
            try {
                $service = new AbsorbedEnergyLimitService($model);
     ...
     * @throws NotFoundHttpException
     */
    private function findModel($id): AbsorbedEnergyLimit
    {
        $model = AbsorbedEnergyLimit::findOne(intval($id));
        if (!$model) {
