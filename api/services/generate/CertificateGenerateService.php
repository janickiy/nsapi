...
use api\models\NonDestructiveTest;
...
class CertificateGenerateService
{
    ...
    protected CertificateGeneratePrepareService $prepareService;
    protected bool $isFillEnergy = true;
    ...
    public function generateCertificate(): Xls
    {
        $certificateName = sprintf(
            ...
            Yii::$app->formatter->asDate($this->certificate->created_at),
        );
    ...
    private function getTemplate(): Spreadsheet
    {
        $filePath = $this->isFillEnergy
            ? Yii::getAlias('@common/cert_templates/common.xlsx')
            : Yii::getAlias('@common/cert_templates/tu01.xlsx');
    ...
    private function fillSheet2(Worksheet $sheet, string $certificateName): void
    {
        ...
        if ($fluidityLimit) {
            $sheet->setCellValue(
                ...
                $this->generateRichText(
                    "не менее "
                        . Yii::$app->formatter->asDecimal($fluidityLimit->value_min)
                        . "\n\nне более "
                        . Yii::$app->formatter->asDecimal($fluidityLimit->value_max),
                    true
                )
            );
        }
        if ($strengthLimit) {
            $sheet->setCellValue(
                ...
                $this->generateRichText("не менее "
                    . Yii::$app->formatter->asDecimal($strengthLimit->value), true)
            );
        }
        if ($minRelativeExtension) {
            $sheet->setCellValue(
                'L10',
                $this->generateRichText("не менее "
                    . Yii::$app->formatter->asDecimal($minRelativeExtension), true)
            );
        }
        if ($hardnessLimit) {
            $sheet->setCellValue(
                ...
                $this->generateRichText("не более "
                    . Yii::$app->formatter->asDecimal($hardnessLimit->value), true)
            );
        }
        if ($minAbsorbedEnergy && $this->isFillEnergy) {
            $sheet->setCellValue(
                'Q11',
                $this->generateRichText("не менее "
                    . Yii::$app->formatter->asDecimal($minAbsorbedEnergy), true)
            );
        }
        if ($minAverageAbsorbedEnergy && $this->isFillEnergy) {
            $sheet->setCellValue(
                'T11',
                $this->generateRichText("не менее "
                    . Yii::$app->formatter->asDecimal($minAverageAbsorbedEnergy), true)
            );
        }
        ...
        foreach ($this->certificate->rolls as $roll) {
            ...
            $sheet->setCellValue(
                "E$currentRow",
                $roll->length ? Yii::$app->formatter->asDecimal($roll->length, 2) : ''
            );
            if ($roll->locationSeams) {
                ...
                $sheet->setCellValue(
                    "F$currentRow",
                    $this->generateRichText(Yii::$app->formatter->asDecimal($roll->locationSeams ,2), false, $ref)
                );
            } else {
            ...
            if ($roll->fluidity) {
                ...
                $sheet->setCellValue(
                    "H$currentRow",
                    $this->generateRichText(Yii::$app->formatter->asDecimal($roll->fluidity ,2), false, $ref)
                );
            } else {
...
//            $sheet->setCellValue(
//                "L$currentRow",
//                $roll->is_fill_testing ? ($roll->relative_extension ?? '') : ''
//            );
            if ($roll->relative_extension) {
                ...
                $sheet->setCellValue(
                    "L$currentRow",
                    $this->generateRichText(Yii::$app->formatter->asDecimal($roll->relative_extension ,2), false, $ref)
                );
            } else {
            ...
            if ($roll->hardness_om) {
                $sheet->setCellValue(
                    "N$currentRow",
                    ($roll->is_exact_hardness_om ? '' : '< ') . Yii::$app->formatter->asDecimal($roll->hardness_om, 2)
                );
            }
            if ($roll->hardness_ssh) {
                $sheet->setCellValue(
                    "O$currentRow",
                    ($roll->is_exact_hardness_ssh ? '' : '< ') . Yii::$app->formatter->asDecimal($roll->hardness_ssh, 2)
                );
            }
            if ($roll->hardness_ztv) {
                $sheet->setCellValue(
                    "P$currentRow",
                    ($roll->is_exact_hardness_ztv ? '' : '< ') . Yii::$app->formatter->asDecimal($roll->hardness_ztv, 2)
                );
            }
            ...
            if (/*$roll->is_fill_testing*/$this->isFillEnergy) {
                ...
                if ($roll->absorbed_energy_1) {
                    $sheet->setCellValue(
                        "Q$currentRow",
                        Yii::$app->formatter->asDecimal($roll->absorbed_energy_1, 2)
                    );
                ...
                if ($roll->absorbed_energy_2) {
                    $sheet->setCellValue(
                        "R$currentRow",
                        Yii::$app->formatter->asDecimal($roll->absorbed_energy_2, 2)
                    );
                ...
                if ($roll->absorbed_energy_3) {
                    $sheet->setCellValue(
                        "S$currentRow",
                        Yii::$app->formatter->asDecimal($roll->absorbed_energy_3, 2)
                    );
                ...
                if ($countEnergy) {
                    $sheet->setCellValue(
                        "T$currentRow",
                        Yii::$app->formatter->asDecimal(round($totalEnergy / $countEnergy, 2), 2)
                    );
                }
            }
    ...
    private function fillSheet3(Worksheet $sheet, string $certificateName): void
    {
        ...
        if ($massFraction) {
            $sheet->setCellValue(
                'B11',
                $this->generateRichText(
                    "не более " . Yii::$app->formatter->asDecimal($massFraction->carbon),
                    true
                )
            );
            $sheet->setCellValue(
                'C11',
                $this->generateRichText(
                    "не более " . Yii::$app->formatter->asDecimal($massFraction->manganese),
                    true
                )
            );
            $sheet->setCellValue(
                'D11',
                $this->generateRichText(
                    "не более " . Yii::$app->formatter->asDecimal($massFraction->silicon),
                    true
                )
            );
            $sheet->setCellValue(
                'E11',
                $this->generateRichText(
                    "не более " . Yii::$app->formatter->asDecimal($massFraction->sulfur),
                    true
                )
            );
            $sheet->setCellValue(
                'F11',
                $this->generateRichText(
                    "не более " . Yii::$app->formatter->asDecimal($massFraction->phosphorus),
                    true
                )
            );
            ...
            $sheet->setCellValue(
                'H11',
                $this->generateRichText(
                    "не более " . Yii::$app->formatter->asDecimal(Certificate::DIRTY_MAX),
                    true
                )
            );
        ...
        foreach ($this->certificate->melds as $meld) {
            ...
            $sheet->setCellValue(
                "B$currentRow",
                $meld->chemical_c ? Yii::$app->formatter->asDecimal($meld->chemical_c, 3) : ''
            );
            $sheet->setCellValue(
                "C$currentRow",
                $meld->chemical_mn ? Yii::$app->formatter->asDecimal($meld->chemical_mn, 3) : ''
            );
            $sheet->setCellValue(
                "D$currentRow",
                $meld->chemical_si ? Yii::$app->formatter->asDecimal($meld->chemical_si, 3) : ''
            );
            $sheet->setCellValue(
                "E$currentRow",
                $meld->chemical_s ? Yii::$app->formatter->asDecimal($meld->chemical_s, 3) : ''
            );
            $sheet->setCellValue(
                "F$currentRow",
                $meld->chemical_p ? Yii::$app->formatter->asDecimal($meld->chemical_p, 3) : ''
            );
            if ($meld->sekv) {
                $ref = null;
                if ($meld->sekv_note) {
                ...
                $sheet->setCellValue(
                    "G$currentRow",
                    $this->generateRichText(Yii::$app->formatter->asDecimal($meld->sekv, 2), false, $ref)
                );
            } else {
            ...
            $sheet->setCellValue(
                "H$currentRow",
                $meld->dirty_type_a ? Yii::$app->formatter->asDecimal($meld->dirty_type_a, 2) : ''
            );
            $sheet->setCellValue(
                "I$currentRow",
                $meld->dirty_type_b ? Yii::$app->formatter->asDecimal($meld->dirty_type_b, 2) : ''
            );
            $sheet->setCellValue(
                "J$currentRow",
                $meld->dirty_type_c ? Yii::$app->formatter->asDecimal($meld->dirty_type_c, 2) : ''
            );
            $sheet->setCellValue(
                "K$currentRow",
                $meld->dirty_type_d ? Yii::$app->formatter->asDecimal($meld->dirty_type_d, 2) : ''
            );
            $sheet->setCellValue(
                "L$currentRow",
                $meld->dirty_type_ds ? Yii::$app->formatter->asDecimal($meld->dirty_type_ds, 2) : ''
            );
            $currentRow++;
        }
    ...
    private function insertNonDestructiveTest(Worksheet $sheet, int $row, NonDestructiveTest $test, ?int $ref): void
    {
        ...
        $sheet->setCellValue(
            "F$row",
            $this->generateRichText($test->controlMethod ? $test->controlMethod->name : '', false, $ref)
        );
        $sheet->setCellValue(
            "I$row",
            $this->generateRichText($test->ndControlMethod ? $test->ndControlMethod->name : '')
        );
        $sheet->setCellValue(
            "L$row",
            $this->generateRichText($test->controlResult ? $test->controlResult->text : '')
        );
    }
}
